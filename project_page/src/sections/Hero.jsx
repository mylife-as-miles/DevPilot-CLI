...
export default function Hero() {
  ...
  const { lang } = useLang();
  const zh = lang === 'zh';
  const wm = `url(${import.meta.env.BASE_URL}assets/images/devpilot-wordmark.png)`;
  ...
  return (
    <section className="hero" id="top">
      ...
      <div className="hero-inner">
        ...
        <h1 className="hero-wordmark" aria-label="DevPilot">
          <span
        ...
        <Reveal delay={0.25} distance={20}>
          <p className="hero-desc">
            {zh
              ? 'DevPilot 把长周期 AI 研究从一次次孤立的尝试，变成一个累积的过程：假设分叉、实验回传证据、洞见向上传播，只有在留出集上验证有效的改进才会被采纳。'
              : 'DevPilot turns long-horizon AI research from isolated attempts into a cumulative process: hypotheses branch, experiments return evidence, insights propagate, and only held-out improvements are promoted.'}
          </p>
        </Reveal>
