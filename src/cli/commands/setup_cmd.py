"""`devpilot setup` — interactive one-time configuration wizard.

A first-time user runs ``devpilot`` (or ``devpilot setup``) and answers a few prompts;
we write ``~/.devpilot/config.yaml`` so subsequent runs need no flags. The wizard is
the interactive sibling of the flag-driven ``devpilot config init`` and shares its
...
def run_setup_wizard(*, force: bool = False) -> bool:
    ...
    :func:`write_user_llm_config` so the file matches ``devpilot config init``.
    """
    ...
    if GLOBAL_CONFIG_FILE.exists() and not force:
        ...
        if not typer.confirm("Overwrite it?", default=False):
            _console.print("[dim]Keeping the existing config. Run `devpilot config show` to view it.[/]")
            return False

    _console.print()
    _console.print("[bold cyan]devpilot setup[/] — let's configure your model (one time).")
    ...
    provider = _select_choice(
        "API type",
        options=[
            ("auto", "let DevPilot detect it — probes Responses API, else chat completions"),
    ...
    _console.print(
        ...
        "([dim]view it anytime with[/] [bold]devpilot config show[/])."
    )
    _console.print("Just run [bold]devpilot[/] to start a session.\n")
    return True
...
def _setup_openai_oauth() -> bool:
    ...
    _console.print(
        f"\n[green]Done.[/] Saved to [bold]{GLOBAL_CONFIG_FILE}[/]. "
        "Just run [bold]devpilot[/] to start a session.\n"
    )
    return True
...
def setup_command(
    ...
    """Interactively configure devpilot (writes ~/.devpilot/config.yaml)."""
