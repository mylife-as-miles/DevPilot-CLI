"""`devpilot run` — chat with the intake agent, then run the coordinator."""
...
_AUTO_CONFIG_NAMES = ("research_config.yaml", "devpilot.yaml", "autoresearch.yaml")
...
def run_command(
    ...
    config: Path | None = typer.Option(
        None, "--config", "-c",
        help="Project YAML config. Defaults to research_config.yaml / devpilot.yaml / autoresearch.yaml in the target project.",
    ),
    ...
    run_name: str | None = typer.Option(None, "--run-name", help="Session name under .devpilot/sessions/. Defaults to timestamp."),
    ...
    # user through `devpilot setup` once instead of failing on missing credentials,
    ...
    # from devpilot setup / project YAML; run-time CLI only overrides execution
    ...
    # compose inline, and nothing flickers. DEVPILOT_DASHBOARD_INPUT_MODE=raw|line
    ...
    requested_dashboard_input_mode = os.environ.get("DEVPILOT_DASHBOARD_INPUT_MODE", "").strip().lower()
    ...
    # The previous "devpilot run" banner panel (task/cwd/model/cycles/...)
...
def _live_input_unsafe_terminal(env: "os._Environ[str] | dict[str, str]") -> bool:
    ...
    ``DEVPILOT_FORCE_DASHBOARD_INPUT`` (1/true/yes) forces raw input back on for
    users who know their setup is fine.
    """
    if env.get("DEVPILOT_FORCE_DASHBOARD_INPUT", "").strip().lower() in ("1", "true", "yes"):
        return False
...
def _resolve_dashboard_input_mode(env: "os._Environ[str] | dict[str, str]") -> str:
    ...
    """
    requested = env.get("DEVPILOT_DASHBOARD_INPUT_MODE", "").strip().lower()
    if requested == "prompt":
        requested = "app"
...
def _add_effective_llm_overrides(overrides: dict[str, Any], eff: dict[str, Any]) -> None:
    """Copy LLM values resolved from devpilot setup/project YAML into config overrides.

    ``devpilot run`` no longer accepts model/provider flags, but the final
    CoordinatorConfig must still receive the values selected by ``devpilot setup``
