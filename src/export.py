"""Session export helpers for DevPilot runs.
...
def resolve_session_dir(session: Path, cwd: Path | None = None) -> Path:
    """Resolve a session path or name under ``<cwd>/.devpilot/sessions``.

    Mirrors the existing ``devpilot report`` resolution behavior, but raises a
...
def _render_html(data: dict[str, Any]) -> str:
    ...
    summary = data["summary"]
    title = summary.get("task") or summary.get("session_name") or "DevPilot session"
    return f"""<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>{_esc_text(title)} - DevPilot Export</title>
  <style>
{_EXPORT_CSS}
  </style>
</head>
<body>
  <div id="app">
    <aside id="sidebar">
      <div class="brand">DevPilot Export</div>
