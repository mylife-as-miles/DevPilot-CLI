...
ChatGPT Plus/Pro/Team subscribers can drive DevPilot with their subscription
...
:class:`devpilot.core.llm.openai_oauth.OpenAIOAuthProvider`.

Tokens live in ``~/.devpilot/oauth/openai.json`` (chmod 600). Using a ChatGPT
...
def refresh(tokens: OpenAITokens) -> OpenAITokens:
    ...
    if not tokens.refresh_token:
        raise OAuthError("no refresh token available; run `devpilot login openai` again")
    data = _post_token({
...
def get_valid_tokens() -> OpenAITokens:
    ...
    if tokens is None:
        raise OAuthError(
            "not logged in to ChatGPT — run `devpilot login openai`"
        )
...
class _CallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 (stdlib signature)
        ...
        self.wfile.write(
            ...
            b"margin-top:4rem'><h2>DevPilot is now signed in to ChatGPT.</h2>"
...
def login(*, open_browser: bool = True, timeout: float = 300.0) -> OpenAITokens:
    ...
    try:
        ...
        print("\nOpen this URL to authorize DevPilot with your ChatGPT account:")
        print(f"\n  {url}\n")
        if not opened:
