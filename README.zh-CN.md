...
<p align="center">
  ...
  <a href="https://github.com/RUC-NLPIR/DevPilot/discussions"><img src="https://img.shields.io/badge/讨论区-加入-5865F2?style=for-the-badge&logo=github&logoColor=white" alt="Discussions"></a>
...
**DevPilot 是一个自主科研智能体，可以把长周期目标转化为持续累积的搜索过程。** 给它一个基准
（benchmark）和一个目标，它会提出假设、修改代码、运行真实实验、从结果中学习，并保留那些在
留出（held-out）数据上经得起验证的改进。不同于“一次性尝试、过后即弃”的做法，DevPilot 会逐步生长出一棵
**假设树**：每个想法都是一根分支，失败则剪枝，成功则保留；洞见会沿树反向传播，让后续想法
从更可靠的起点出发。

更多详情，请访问我们的[项目主页](https://ruc-nlpir.github.io/DevPilot/)并阅读[论文](https://arxiv.org/pdf/2606.11926)。如需详细的使用说明，请参阅[文档](https://ruc-nlpir.github.io/DevPilot/docs/)。🧭 你也可以根据自己的环境和工作流选择使用 [CLI 版本或技能套件版本](https://claude.ai/chat/e7121091-ce2c-4970-a60f-16b54c453729#-cli-与技能套件版本)。

## 📣 最新动态

- **2026-06** — DevPilot 被美国知名科技媒体 [VentureBeat](https://venturebeat.com/) 报道：[《New AI optimization framework beats Claude Code and Codex by 2.5x on the same compute budget》](https://venturebeat.com/orchestration/new-ai-optimization-framework-beats-claude-code-and-codex-by-2-5x-on-the-same-compute-budget)。📰
- **2026-06** — DevPilot 原生 CLI 运行时与智能体技能套件（Codex / Claude Code）正式发布。🚀
- **2026-06** — DevPilot 论文在 [arXiv](https://arxiv.org/abs/2606.11926) 发布。🎉

## 💡 为什么选择 DevPilot

- **通用优化能力** — 无论是模型训练、评测工程，还是数据合成，只要有明确的优化目标和可量化的评估指标，DevPilot 都能胜任。
- **真正落地可用的自主科研** — DevPilot 不仅是一个研究原型，它同时提供原生 CLI 运行时和适用于 Codex 与 Claude Code 的智能体技能套件。你可以直接使用完整 CLI 以获得最佳效果，也可以在其他编程智能体中加载技能套件来使用。
- **长期结构化探索** — 假设树框架让 DevPilot 能够持续运行，进行累积式搜索：实验结果、失败原因和提炼出的洞察都会保存在想法树中并向上传播，让后续想法越来越聪明，而不是淹没在无尽的上下文滚动里。
...
- **灵活的模型与工作流支持** — DevPilot 通过 LiteLLM 支持 Anthropic、OpenAI / Responses API 及 OpenAI 兼容后端，包括 DeepSeek、Gemini、Qwen、vLLM、Ollama 和本地网关。
- **高度可控与可扩展** — 实时终端仪表盘、只读 WebUI、可选的人工审核介入，以及一行配置即可切换的领域插件，让你无需修改 DevPilot 核心代码就能灵活调整实验走向。

## 🧩 框架原理

<p align="center">   <img src="assets/framework.png" alt="DevPilot 框架" width="100%"> </p>

DevPilot 由**两个协同工作的智能体**组成：

- **协调器（Coordinator）** — 研究总监。负责维护想法树、驱动 DevPilot 循环并下发实验任务。
- **执行器（Executor）** — 研究工程师。接收单个想法后，忠实地实现代码变更，在隔离的 git 工作树中运行实验，并汇报实验证据。

两者共同重复执行六步 **DevPilot 循环**：
...
本仓库提供两种使用 DevPilot 的方式：

| 版本            | 位置                                                 | 适用场景                                                     | 建议                                                        |
| --------------- | ---------------------------------------------------- | ------------------------------------------------------------ | ----------------------------------------------------------- |
| 原生 CLI 运行时 | Python 包及 `devpilot` 命令                             | 真实的 DevPilot 研究运行、长时间实验、仪表盘、检查点、执行器工具、合并/测试纪律、插件、报告 | **推荐使用**。功能更完整、更可靠，能发挥 DevPilot 的最佳性能。 |
| 智能体技能套件  | [`skills/`](https://claude.ai/chat/skills/README.md) | 在 Codex 或 Claude Code 环境中获得 DevPilot 风格的行为，无需运行原生 DevPilot 运行时 | 实用的集成层和备用方案，但功能不如 CLI 运行时完整。         |

如果你能运行 CLI，就使用 CLI。原生 `devpilot` 运行时包含完整实现：任务摄入、研究合同、实时仪表盘、事件总线、检查点与恢复、执行器派发、受保护的开发/测试评估纪律、搜索智能体、插件和最终报告生成。

仓库根目录下的 [`skills/`](https://claude.ai/chat/skills/README.md) 目录是一套 Codex/Claude Code 技能套件。安装完成后，在 Codex 中调用 `$devpilot-research-agent`，或在 Claude Code 中调用 `/devpilot-research-agent`，然后像描述 DevPilot 任务一样描述你的研究目标即可。当目标、指标、数据、权限、预算或运行模式不明确时，技能套件会先进行 DevPilot 风格的澄清确认，再加载协调器和各阶段技能。该目录与存放在 `src/skills/` 下的内部运行时技能是分开的。
...
```bash
pip install devpilot-agent   # 或使用：uv pip install devpilot-agent
devpilot doctor              # 验证 PATH、git 及 API 密钥
```

> 想要全局命令？使用 `pipx install devpilot-agent` 可将 `devpilot` 命令安装至全局环境。
...
```bash
devpilot setup       # 首次使用：配置模型提供商 / 模型 / base_url / API 密钥
devpilot             # 在当前目录启动交互式会话
devpilot doctor      # 诊断安装状态
```

`devpilot setup` 会将配置写入 `~/.devpilot/config.yaml`，此后日常使用直接运行 `devpilot` 即可，无需额外参数。DevPilot 启动后首先进行一次**任务摄入对话**，将你的目标、目标目录、评估指标、基线、预算、开发/测试纪律和产物路径整理成一份简洁的 **DevPilot 研究合同**。确认后，实时仪表盘随即接管。
...
devpilot "提升验证集分数，不得修改测试集" --cwd ./benchmark
...
```yaml
# LLM/API 配置由 `devpilot setup` 管理；项目配置通常只需指定任务和预算。
task: >
  优化智能体在该基准上的准确率。
  不得修改评估框架或数据文件。

coordinator:
  max_cycles: 10          # DevPilot 循环轮数
  max_depth: 2            # 想法树深度
...
如果你只想完整地看一遍 DevPilot 是怎么工作的——**不耗 API 预算、不需要 GPU**——
...
```bash
cp -r examples/algotune_knn /tmp/algotune_knn   # 在 DevPilot 仓库之外运行
cd /tmp/algotune_knn
...
通过 `devpilot setup` 一次性配置 LLM 访问（存储在 `~/.devpilot/config.yaml`），只需填写一个 `provider` 字段：

| `provider` | 适用场景 |
| --- | --- |
| `auto`（默认） | 交给 DevPilot 自动判断：探测端点的 OpenAI **Responses** API，支持就用（保留思维链），否则回退到 chat completions；Claude 模型走原生 Anthropic API。探测结果会被冻结写入配置。 |
...
大多数用户只需运行 `devpilot setup`、保持 `auto`、填好 `model` 与 `base_url` 即可。API 密钥从环境变量或配置文件中读取；针对具体项目的任务和预算配置则放在 `research_config.yaml` 中。详情请参阅[配置指南](https://ruc-nlpir.github.io/DevPilot/docs/configuration/)和 [`examples/research_config.example.yaml`](examples/research_config.example.yaml)。
...
| ------------------------ | ----------------------------------------------------------- |
| `devpilot`                  | 启动交互式研究会话。                                        |
| `devpilot setup`            | 配置模型提供商 / 模型 / API 密钥 → `~/.devpilot/config.yaml`。 |
| `devpilot report <session>` | 重新渲染某次历史会话的 `REPORT.md`。                        |
| `devpilot export <session> [output]` | 将历史会话导出为自包含 HTML；当 `output` 以 `.jsonl` 结尾时导出 JSONL。 |
| `devpilot doctor`           | 诊断安装状态、PATH、git 及 API 密钥。                       |
| `devpilot version`          | 打印已安装的版本号。                                        |

底层入口点（`run-research`、`coordinator`、`executor`、`review-research`）保留供调试使用——详见 [CLI 参考文档](https://ruc-nlpir.github.io/DevPilot/docs/cli/)。
...
每次运行会在 `.devpilot/sessions/` 下创建一个会话目录，包含 `REPORT.md`、`events.jsonl`、`run_stats.json`、想法树以及各实验的产物文件。运行支持断点恢复——用 `Ctrl+C` 中断后，稍后使用 `--resume` 继续；DevPilot 会重新加载想法树并从中断处继续。

```bash
devpilot report .devpilot/sessions/<run_name>   # 重新渲染历史报告
devpilot export <run_name>                   # 写入 .devpilot/sessions/<run_name>/devpilot-session-<run_name>.html
devpilot export <run_name> session.jsonl     # 导出 JSONL 产物包
devpilot --resume --run-name <run_name>      # 继续中断的运行
```
...
DevPilot 作为**单一控制器**，在模型训练、harness 工程和数据合成等场景中进行评测；实验中只改变材料、
目标、评测器和预算。在全部六项任务上，DevPilot 都在留出 test 上超过了强大的单智能体基线。

| 任务               | 优化方向 | 初始值 | Codex | Claude Code | **DevPilot**  | 提升   |
| ------------------ | -------- | ------ | ----- | ----------- | ---------- | ------ |
...
在使用 GPT-5.5 的 **MLE-Bench Lite** 上，DevPilot 达到了 **86.36% 任意奖牌率**（100% 有效提交，95.45% 超过中位数，77.27% 获得金奖）。完整的实验协议和消融分析请参阅[论文](https://arxiv.org/pdf/2606.11926)。

------

## 🗂️ 项目结构

代码位于 `src/`，以 `devpilot` 包的形式导入。
...
├── cli/             `devpilot` CLI：intake、实时仪表盘、setup、doctor、config
...
claw-code 是 Claude Code 的开源 Rust 复现，为 DevPilot CLI 提供了 REPL 框架、
