...
!!! tip "In a hurry? You can skip this page"
    Most people configure DevPilot exactly once: run `devpilot setup`, pick a model, done.
...
This page is written for someone who has never used DevPilot. It answers three questions, in
order:
...
=== "Just trying it out"
    ...
    Run `devpilot setup` to choose a model, then `devpilot` to start. You do **not** need a
...
=== "Running a real study"
    ...
    Do `devpilot setup`, then skim [What you can configure](#what-you-can-configure) and
...
| --- | --- | --- | --- |
| **Essential** | `provider`, `model`, `api_key`, `base_url` | Which LLM DevPilot uses and how to reach it | You must pick a model and supply a key once. |
| **Important** | `max_cycles` | How many experiments before DevPilot stops and writes the report | The main time/cost knob. Higher = longer, deeper search. |
...
1. **`devpilot setup`** — a one-time wizard that saves your model globally. *Most people only
   ever use this.*
2. **`devpilot config`** — view or edit that global file later.
...
```console
$ devpilot setup
devpilot setup — let's configure your model (one time).
...
Done. Saved to ~/.devpilot/config.yaml
```
...
After this, just run `devpilot`.

### 2. Inspect or edit the global config: `devpilot config`

```bash
devpilot config show           # print the effective config (secrets masked)
devpilot config path           # where the file lives
devpilot config init --provider openai --model gpt-5 --api-key dummy   # write it non-interactively
```

`devpilot config init` is the scriptable sibling of the wizard — handy for setting up a local
gateway in one line:

```bash
devpilot config init --provider litellm --model qwen-72b \
  --base-url http://localhost:4141 --api-key dummy
```

### Providers

Pick **one** API type. The value you give to `devpilot setup` / `--provider` is one of exactly
three:
...
!!! tip "Keep keys out of files"
    ...
    file. `devpilot setup` stores your global key under `~/.devpilot/` with the rest of the config.

!!! warning "Experimental: ChatGPT subscription login (`openai-oauth`)"
    ChatGPT Plus/Pro/Team subscribers can drive DevPilot with their subscription instead of a
    pay-per-token key. Run `devpilot login openai` to sign in through the browser; the token is
    stored in `~/.devpilot/oauth/openai.json` and refreshed automatically. This writes:
    ...
    Manage the session with `devpilot login status` / `devpilot login logout`. Requests go to the
...
When a project needs its own durable settings, drop a YAML file in it. DevPilot auto-detects
`research_config.yaml`, `devpilot.yaml`, or `autoresearch.yaml` in the target directory (or
...
# ── Orchestration ─────────────────────────────────────
max_cycles: 12                   # experiments before DevPilot finalizes and reports
...
```bash
devpilot run --max-cycles 20 --mode review --no-webui
```

Common ones: `--max-cycles N`, `--max-turns N`, `--mode MODE`, `--webui-port N`,
`--no-webui`. See the [CLI reference](cli.md#devpilot-run) for the full list.
...
| Key | Meaning |
| --- | --- |
| `max_cycles` | Maximum number of completed / skipped / failed idea experiments before DevPilot finalizes and writes the report. Override per-run with `--max-cycles`. |
...
Two top-level keys retarget DevPilot to a domain without touching code:
...
```text
built-in defaults  <  plugin overrides  <  plugin profile  <  global setup (~/.devpilot)  <  project config  <  CLI flags
```
...
```bash
devpilot config show   # confirm provider/model/key are what you expect (secrets masked)
devpilot doctor        # check PATH, Python, git, and that your API key resolves
```

`devpilot doctor` is the fastest way to catch a missing key or unreachable gateway before a
run starts.
