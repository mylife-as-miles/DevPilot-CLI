...
DevPilot 记录一次运行产生的一切，让你能检视它、复现它，并从你停下的地方继续。
...
!!! tip "复用一套设置"
    ...
    并命名为 `devpilot.yaml`——DevPilot 下次会自动加载它。见
    [准备基准 → 保存并复用一套设置](preparing-a-benchmark.md#saving-and-reusing-a-setup)。

## `REPORT.md`

一次运行结束时，DevPilot 写出一个 `REPORT.md`——这项研究的人类可读成稿：尝试了什么、什么奏效、
每个结论背后的证据，以及最终结果。用 `devpilot report` 处理它，运行过程中用 `/report` 斜杠命令
打印产物路径。

默认情况下 DevPilot 随后会打开一个**只读问答提示**，让你就这次已完成的运行进行追问（用 `--no-followup`
关闭）。
