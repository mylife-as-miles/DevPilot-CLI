---
name: devpilot-agent-tools
description: "Deterministic helper layer for emulating DevPilot tools in Codex or Claude Code when native TreeView, TreeAddNode, TreeSetMeta, RunExecutor, GitMergeBranch, or report tooling is unavailable. Use for local state management, eval score capture, executor prompt generation, merge checks, and skill forward tests."
---

# DevPilot Agent Tools

Use this skill when the host does not provide native DevPilot tools. The bundled
script stores state in the same style as open-source DevPilot:

```text
<cwd>/.devpilot/sessions/<run_name>/.coordinator/idea_tree.json
<cwd>/.devpilot/sessions/<run_name>/.coordinator/idea_tree.md
```

## Script

`scripts/devpilot_state.py` is stdlib-only.
...
DevPilot repository, initialize a short run, record metadata, add one idea,
