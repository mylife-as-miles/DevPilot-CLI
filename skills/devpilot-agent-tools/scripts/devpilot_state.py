#!/usr/bin/env python3
"""Small stdlib helper for DevPilot-style local research state.

This is not the real DevPilot runtime. It preserves the open-source session/tree
...
def session_dir(cwd: str | Path, run_name: str) -> Path:
    return Path(cwd).resolve() / ".devpilot" / "sessions" / run_name
...
def executor_prompt(
    ...
    if smoke:
        parts.append(
            "## Smoke Mode\n\n"
            "This is a forward-test of DevPilot orchestration only. Do not edit "
        ...
        parts.append(
            ...
            "2. Use `devpilot_state.py parse-log` or a small parser for cached "
            ...
            f"Save smoke artifacts under `.devpilot/sessions/<run>/experiments/{node_id}/`."
        )
    else:
        parts.append(
            "## Instructions\n\n"
...
def cmd_worktree(args: argparse.Namespace) -> None:
    ...
    base = Path(tempfile.gettempdir()) / f"devpilot-worktrees-{os.getuid()}"
...
def cmd_merge(args: argparse.Namespace) -> None:
    ...
    if test_score is None and eval_cmd_test:
        base = Path(tempfile.gettempdir()) / f"devpilot-merge-eval-{os.getuid()}"
...
def add_common(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--run-name", default="skill_run", help="session name under .devpilot/sessions")
