...
Use this file when auditing the skill suite against the current DevPilot source
tree.
...
- `src/cli/commands/run.py`: `devpilot run`, intake, preflight,
...
- `src/coordinator/prompts.py`: coordinator identity and full DevPilot
  cycle protocol.
...
- The open-source branch uses the `devpilot` CLI, intake planning, session
