...
# DevPilot Research Agent Skill Suite（中文）

本目录包含一套 Codex/Claude Code skill suite，用 Agent Skills 的形式复刻
`devpilot` open-source 分支中的 DevPilot/AutoResearch 行为。

大多数用户只需要调用公开入口：

```text
$devpilot-research-agent <你的研究或优化需求>
```
...
内部阶段 skill 仍然必须安装。请把所有 `devpilot-*` skill 目录作为一整套安装，不要只
安装 `devpilot-research-agent`。

## 快速下载与安装

把 `REPO_URL` 设置成 DevPilot GitHub 仓库地址，把 `REPO_REF` 设置成包含这个
`skills/` 目录的分支或 tag。
...
这套 skill suite 是可用的，并且在 Agent Skills 能表达和执行的层面上对齐 DevPilot 的
核心行为。

它覆盖：

- 类似 `devpilot run` 的公开 intake 入口。
- 对目标、metric、数据、eval、权限、预算、run mode 缺失时的 DevPilot 式反问。
- 用户已给出足够约束或明确使用安全 defaults 时的 fast path。
- 从 wrapper 传给 orchestrator 的 research contract。
- 阶段加载式 orchestrator，而不是一个单体大 skill。
- 持久化 `.devpilot/sessions/<run_name>/` session state。
...
- IDEATE、executor、merge/eval、related-work search、plugin/HITL/budget、
  resume、report 等 DevPilot 行为面。
- 在没有原生 DevPilot tools 的 Codex/Claude 环境中，用 deterministic helper
  `devpilot_state.py` 模拟核心状态工具。

边界也要明确：这不是原生 `devpilot` CLI runtime 的二进制替代品。原生 dashboard、
EventBus streaming、provider runtime、完整原生 executor 并发、后台 SearchAgent
生命周期仍属于原项目 runtime。如果机器上已安装原生 `devpilot` CLI，并且目标是生产级
完整 DevPilot run，应优先使用原生 runtime。这套 skill suite 的目标是在没有原生 DevPilot
tools，或需要 skill-based reconstruction 的环境中，让 Codex/Claude Code 按 DevPilot 的
设计行动。
...
| Skill | 责任 |
| --- | --- |
| `devpilot-research-agent` | 公开入口。做 DevPilot 式 intake/clarification，形成 research contract，然后加载 orchestrator。 |
| `devpilot-agent-orchestrator` | 顶层 phase loader 和 policy owner，决定何时加载各阶段 skill。 |
| `devpilot-agent-setup-intake` | 项目 intake、metric/eval 发现、baseline、B_dev/B_test policy、session setup。 |
| `devpilot-agent-coordinator` | INIT/OBSERVE/IDEATE/SELECT/DISPATCH/DECIDE 大循环和 durable Idea Tree。 |
| `devpilot-agent-ideate` | 复刻 idea drafting 和 first-principles probing，强制 constraints view 和四行 hypothesis。 |
| `devpilot-agent-executor` | executor/worktree/prompt/report/metrics/insight propagation 行为。 |
| `devpilot-agent-merge-eval` | B_dev/B_test 隔离、merge guard、protected paths、metric direction、final scoring。 |
| `devpilot-agent-search` | 对 validated winner 做 related-work 和 novelty search。 |
| `devpilot-agent-plugins-hitl-budget` | plugin/profile 优先级、MLE/Kaggle 行为、HITL gates、budget/cycle policy。 |
| `devpilot-agent-resume-report` | checkpoint/resume、running-node requeue、`REPORT.md` finalization。 |
| `devpilot-agent-tools` | 没有原生 DevPilot tools 时的 deterministic fallback tools。 |
...
- `devpilot-agent-orchestrator/references/source-map.md`：从 `devpilot`
  open-source 分支到本 suite 的源码级映射。
- `devpilot-agent-orchestrator/references/compatibility.md`：Codex 和 Claude Code
  兼容性说明。
- `devpilot-agent-tools/references/tool-mapping.md`：原生 DevPilot tools 和 fallback helper
  commands 的映射。
- `devpilot-agent-tools/scripts/devpilot_state.py`：stdlib-only helper，支持 `init`、
  `view`、`meta`、`add`、`update`、`prune`、`propagate`、`eval`、`parse-log`、
  `prompt-executor`、`record`、`worktree`、`merge`、`check`、`report`。

## DevPilot 行为映射

| DevPilot 行为 | skill suite 对应 |
| --- | --- |
| `devpilot run` 先进入 intake 并整理 Research Contract | `devpilot-research-agent` |
| `.devpilot/sessions/<run_name>/` session layout | `devpilot-agent-setup-intake` + `devpilot-agent-tools` |
| 持久 coordinator ReAct loop | `devpilot-agent-orchestrator` + `devpilot-agent-coordinator` |
| `TreeView`, `TreeAddNode`, `TreeSetMeta`, `TreeUpdateNode`, `TreePropagate` | `devpilot-agent-coordinator` + `devpilot_state.py` |
| IDEATE 前必须 `TreeView(format="constraints")` | `devpilot-agent-ideate` |
| 四行 hypothesis：`Mechanism`, `Hypothesis`, `Observable`, `Conflicts` | `devpilot-agent-ideate` |
| `RunExecutor` / `RunExecutorParallel` | `devpilot-agent-executor` |
| Executor 使用 B_dev，不碰 B_test | `devpilot-agent-executor` + `devpilot-agent-merge-eval` |
| `GitMergeBranch` 自动做 B_test verification 和 protected-path checks | `devpilot-agent-merge-eval` + `devpilot_state.py merge` |
| SearchAgent 只标注 validated winners | `devpilot-agent-search` |
| plugin/profile/HITL/budget policy | `devpilot-agent-plugins-hitl-budget` |
| checkpoint/resume/final report | `devpilot-agent-resume-report` + `devpilot_state.py report` |
| 长训练和噪声 progress logs | `devpilot-agent-executor` + `devpilot_state.py parse-log` |
...
- orchestrator 初始化 `.devpilot/sessions/<run_name>/`。
...
这对应原生 DevPilot intake 和 Research Contract 的体验。

## 运行期保护规则

- 必须安装所有 `devpilot-*` skill 目录，不要只安装 `devpilot-research-agent`。
...
这套 suite 已经过静态检查和动态 Codex smoke run 验证；动态验证应在 DevPilot checkout
之外的临时目标仓库中进行。一次可复现的验证应确认：

- 11 个 skill 全部通过 `quick_validate.py`。
- 11 个 `agents/openai.yaml` 都可解析。
- `devpilot_state.py` 可编译。
- `devpilot_state.py check` 在带上预期 artifact flags 时返回 `OK`。
- 新 Codex 只从 `$devpilot-research-agent` 入口启动时，能完成 intake，加载
  orchestrator 和各阶段 skill，维护 Idea Tree，通过 executor-style 行为派发，并生成
  DevPilot-style artifacts。
...
通常是只复制了 `devpilot-research-agent`。需要复制所有 `devpilot-*` skill 目录，并重启
Codex 或 Claude Code。

### Claude Code 没有触发 `/devpilot-research-agent`

检查安装路径：

- 用户级：`~/.claude/skills/devpilot-research-agent/SKILL.md`
- 项目级：`<target_repo>/.claude/skills/devpilot-research-agent/SKILL.md`
...
### Codex 没有触发 `$devpilot-research-agent`
...
`devpilot-research-agent` 开始。

### 什么时候应该使用原生 DevPilot？

如果需要生产级完整 DevPilot run，且原生 `devpilot` CLI 已安装，优先使用原生 runtime。
这套 skill suite 更适合：

- 在 Codex 或 Claude Code 中复刻 DevPilot-style 行为。
- 没有原生 DevPilot tools 的环境。
- smoke 或 forward test。
- 教 agent 遵守 DevPilot 的研究纪律。
- 共享跨平台的 `SKILL.md` 工作流。
